...
{
    return row >= 0 && row < GameConfig::ROW_COUNT && column >= 0 && column < GameConfig::COLUMN_COUNT;
}
...
int Board::ClearCompletedLines()
{
    ...
    for (int row = GameConfig::ROW_COUNT - 1; row >= 0; --row)
    {
...
const std::array<std::array<Board::CellState, GameConfig::COLUMN_COUNT>, GameConfig::ROW_COUNT> &Board::Cells() const
{
    return boardCells;
}